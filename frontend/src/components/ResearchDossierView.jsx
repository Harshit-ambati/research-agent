...
export default function ResearchDossierView({
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      {/* Top Header Card */}
      <div className="relative rounded-2xl bg-gradient-to-r from-slate-900/90 via-dark-900/90 to-slate-900/90 border border-slate-700/60 p-6 backdrop-blur-xl shadow-2xl overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2.5 mb-2">
              <span className="px-2.5 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" />
                Autonomous Research Dossier
              </span>
              <span className="px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/30 text-xs font-medium">
              ...
              {llm_powered ? (
                ...
                <span className="px-2 py-0.5 rounded-md bg-slate-800 text-slate-400 text-[11px] border border-slate-700">
            ...
            <p className="text-slate-400 text-sm mt-1 max-w-3xl">
              Cross-disciplinary investigation synthesizing academic literature, patents, open datasets, and implementation frameworks for <span className="text-cyan-300 font-medium">"{query}"</span>.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-black font-semibold text-sm transition-all shadow-lg shadow-cyan-500/20 active:scale-95"
            >
        ...
        {chain_of_thought.length > 0 && (
          <div className="mt-5 pt-4 border-t border-slate-800/80">
            <button
              ...
              className="flex items-center justify-between w-full text-left text-xs font-medium text-slate-400 hover:text-cyan-300 transition-colors"
            >
              <span className="flex items-center gap-2">
                <Cpu className="w-4 h-4 text-cyan-400 animate-pulse" />
                <span className="font-semibold text-slate-300">Agent Cognitive Reasoning:</span> 4-Stage Multi-Source Corroboration
              </span>
              {isCotExpanded ? (
            ...
            {isCotExpanded && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
                {chain_of_thought.map((step, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-xl bg-dark-950/70 border border-slate-800 text-xs space-y-1.5"
                  >
      ...
      {/* Navigation Tabs Bar */}
      <div className="flex overflow-x-auto gap-2 p-1.5 rounded-2xl bg-dark-900/80 border border-slate-800 backdrop-blur-md scrollbar-none">
        {[
          { id: 'synthesis', label: 'Research Dossier', icon: Sparkles, count: null },
          { id: 'papers', label: 'Academic Papers', icon: BookOpen, count: papers.length },
          { id: 'patents', label: 'Patents & IP', icon: Lightbulb, count: patents.length },
          ...
          { id: 'challenges', label: 'Applied Challenges', icon: Target, count: problem_statements.length },
      ...
      {activeTab === 'synthesis' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 p-6 rounded-2xl bg-dark-900/60 border border-slate-800/80 backdrop-blur-md space-y-4">
            ...
            <div className="p-6 rounded-2xl bg-dark-900/60 border border-slate-800/80 backdrop-blur-md space-y-4">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 rounded-2xl bg-dark-900/60 border border-slate-800/80 backdrop-blur-md space-y-3">
              ...
              <ul className="space-y-2.5">
                {(dossier.key_findings || []).map((finding, idx) => (
                  <li
                    key={idx}
                    className="p-3 rounded-xl bg-dark-950/60 border border-slate-800/80 text-xs text-slate-300 flex items-start gap-2.5"
                  >
              ...
              <ul className="space-y-2.5">
                {(dossier.open_challenges || []).map((challenge, idx) => (
                  <li
                    ...
                    className="p-3 rounded-xl bg-dark-950/60 border border-slate-800/80 text-xs text-slate-300 flex items-start gap-2.5"
                  >
                    <span className="w-5 h-5 rounded-full bg-amber-500/10 text-amber-400 flex items-center justify-center font-bold text-[10px] shrink-0 mt-0.5">
                      !
                    </span>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(dossier.recommended_stack.core_frameworks || []).map((f, i) => (
                        <span key={i} className="px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 text-cyan-300 font-medium">
                          {f}
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(dossier.recommended_stack.datasets_models || []).map((d, i) => (
                        <span key={i} className="px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 text-blue-300 font-medium">
                          {d}
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(dossier.recommended_stack.hardware_infra || []).map((h, i) => (
                        <span key={i} className="px-2.5 py-1 rounded-md bg-slate-800 border border-slate-700 text-purple-300 font-medium">
                          {h}
                        </span>
                      ))}
                    </div>
            ...
            <div className="lg:col-span-2 p-6 rounded-2xl bg-dark-900/60 border border-slate-800/80 backdrop-blur-md space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(dossier.research_roadmap || []).map((phase, idx) => (
                  <div
                    key={idx}
                    className="p-3.5 rounded-xl bg-dark-950/60 border border-slate-800/80 text-xs space-y-2"
                  >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {papers.map((paper, idx) => (
              <div
                key={idx}
                className="p-5 rounded-2xl bg-dark-900/70 border border-slate-800 hover:border-cyan-500/40 transition-all flex flex-col justify-between space-y-3"
              >
                <div>
                  ...
                  {paper.research_takeaway && (
                    <div className="mt-3 p-2.5 rounded-xl bg-cyan-950/40 border border-cyan-800/40 text-xs text-cyan-200 flex items-start gap-2">
                      <Sparkles className="w-3.5 h-3.5 text-cyan-400 shrink-0 mt-0.5" />
                      <div>
                        <span className="font-semibold text-cyan-300">Methodology Takeaway: </span>
                        {paper.research_takeaway}
                      </div>
                    </div>
                  )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {patents.map((patent, idx) => (
              <div
                key={idx}
                className="p-5 rounded-2xl bg-dark-900/70 border border-slate-800 hover:border-amber-500/40 transition-all flex flex-col justify-between space-y-3"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 text-xs mb-1.5">
                    <span className="px-2.5 py-0.5 rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20 font-semibold">
                  ...
                  {patent.novelty_advisory && (
                    <div className="mt-3 p-2.5 rounded-xl bg-amber-950/30 border border-amber-800/40 text-xs text-amber-200 flex items-start gap-2">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {datasets.map((ds, idx) => (
                <div
                  key={idx}
                  className="p-4 rounded-2xl bg-dark-900/70 border border-slate-800 hover:border-cyan-500/40 transition-all flex flex-col justify-between space-y-3"
                >
                  <div>
                    <div className="flex items-center justify-between text-xs text-slate-400 mb-1">
                      <span className="text-cyan-400 font-medium">{ds.author}</span>
                      {ds.downloads && <span>📥 {ds.downloads}</span>}
                    </div>
                    <h3 className="text-sm font-semibold text-white truncate">{ds.name}</h3>
                    <p className="text-xs text-slate-400 mt-1.5 line-clamp-2">{ds.description}</p>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-1.5 w-full py-2 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 text-xs font-semibold transition-colors border border-cyan-500/30"
                  >
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {repositories.map((repo, idx) => (
                <div
                  key={idx}
                  className="p-4 rounded-2xl bg-dark-900/70 border border-slate-800 hover:border-purple-500/40 transition-all flex flex-col justify-between space-y-3"
                >
                  <div>
                    <div className="flex items-center justify-between text-xs text-slate-400 mb-1">
                      <span className="text-purple-400 font-medium">{repo.language || 'Code'}</span>
                    ...
                    <p className="text-xs text-slate-400 mt-1.5 line-clamp-2">{repo.description}</p>
                  </div>
                  <a
                    href={repo.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-1.5 w-full py-2 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 text-xs font-semibold transition-colors border border-purple-500/30"
                  >
                    View on GitHub
                    <ExternalLink className="w-3.5 h-3.5" />
                  </a>
        ...
        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-dark-900/60 border border-slate-800/80 backdrop-blur-md space-y-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="px-2.5 py-1 rounded-md bg-blue-500/10 text-blue-300 border border-blue-500/20 text-xs font-medium uppercase tracking-wider">
              ...
              {knowledge.thumbnail && (
                <img
                  ...
                  alt={knowledge.title}
                  className="w-24 h-24 rounded-xl object-cover border border-slate-700 shadow-md shrink-0"
                />
              )}
            </div>

            <div className="text-slate-300 text-sm leading-relaxed border-t border-slate-800 pt-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {knowledge.related_topics.map((t, i) => (
                  <div key={i} className="p-3.5 rounded-xl bg-dark-950/60 border border-slate-800 text-xs space-y-1">
      ...
      {activeTab === 'challenges' && (
        <div className="space-y-4">
          ...
          {problem_statements.length === 0 ? (
            <div className="p-8 text-center rounded-2xl bg-dark-900/40 border border-slate-800 text-slate-400 text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {problem_statements.map((ps, idx) => (
                <div
                  key={idx}
                  className="p-5 rounded-2xl bg-dark-900/70 border border-slate-800 hover:border-emerald-500/40 transition-all flex flex-col justify-between space-y-3"
                >
                  <div>
      ...
      {activeTab === 'chat' && (
        <div className="p-6 rounded-2xl bg-dark-900/70 border border-slate-800/80 backdrop-blur-md space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {quickChatPrompts.map((p, i) => (
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 text-slate-300 text-xs transition-colors border border-slate-700/60"
              >
                {p}
              </button>
            ))}
          ...
          {/* Chat Messages Log */}
          <div className="min-h-[280px] max-h-[480px] overflow-y-auto space-y-3.5 pr-2">
            {chatMessages.length === 0 ? (
              ...
              chatMessages.map((msg, idx) => (
                ...
                >
                  {msg.role === 'assistant' && (
                    <div className="w-7 h-7 rounded-xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center shrink-0 border border-cyan-500/40 mt-1">
                  ...
                  <div
                    className={`p-3.5 rounded-2xl max-w-2xl ${
                      msg.role === 'user'
                        ...
                        : 'bg-dark-950 border border-slate-800 text-slate-200 rounded-tl-sm prose prose-invert max-w-none'
                    }`}
                  >
                    {renderMarkdown(msg.content)}
                  </div>
                  {msg.role === 'user' && (
                    <div className="w-7 h-7 rounded-xl bg-blue-500/20 text-blue-400 flex items-center justify-center shrink-0 border border-blue-500/40 mt-1">
            ...
            {isChatLoading && (
              <div className="flex gap-3 items-center text-xs text-cyan-400">
                <div className="w-7 h-7 rounded-xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center shrink-0 border border-cyan-500/40 animate-pulse">
                  <Bot className="w-4 h-4" />
                </div>
                <span className="animate-pulse">Synthesizing answer from literature & patents...</span>
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSendChat} className="flex gap-2 pt-2 border-t border-slate-800">
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl bg-dark-950 border border-slate-700/80 text-white text-xs placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition-colors"
            />
            <button
              type="submit"
              disabled={isChatLoading || !chatInput.trim()}
              className="px-4 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 disabled:opacity-50 text-black font-semibold text-xs flex items-center gap-1.5 transition-all"
            >
